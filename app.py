...
train_features = artifact["train_features"].loc[:, feature_names]
...
@st.cache_resource
def prepare_leverage_domain(training_features):
    """Prepare the standardized training design matrix used for leverage AD."""
    ...
    # The pseudo-inverse keeps the calculation stable if descriptors are correlated.
    xtx_inverse = np.linalg.pinv(x_train_augmented.T @ x_train_augmented)
    train_leverage = np.einsum(
        "ij,jk,ik->i", x_train_augmented, xtx_inverse, x_train_augmented
    )
    p = x_train.shape[1]
    n = x_train.shape[0]
    threshold = 3 * (p + 1) / n
    return scaler, xtx_inverse, train_leverage, threshold, p, n


ad_scaler, ad_xtx_inverse, train_leverage, ad_threshold, ad_p, ad_n = prepare_leverage_domain(
    train_features
)
...
def predict_frame(features):
    ...
    out.insert(1, "AD_Leverage", [r["leverage"] for r in ad_rows])
    out.insert(2, "Within_AD", [r["within_ad"] for r in ad_rows])
    out.insert(3, "AD_Threshold", ad_threshold)
    return out, ad_rows


def predict_one_feature_row(feature_row):
    aligned = align_descriptor_frame(feature_row, feature_names)
    pred = float(model.predict(aligned)[0])
    ad = applicability_domain(aligned.iloc[[0]])
    return {
        "Predicted_pIC50": pred,
        "AD_Leverage": ad["leverage"],
        "Within_AD": "Yes" if ad["within_ad"] else "No",
        "AD_Threshold": ad_threshold,
    }


def applicability_domain(feature_row):
    """Assess descriptor-space coverage with the Williams leverage criterion."""
    query = feature_row.loc[:, feature_names].astype(float)
    query_scaled = ad_scaler.transform(query)
    query_augmented = np.column_stack((np.ones(query_scaled.shape[0]), query_scaled))
    leverage = float(
        np.einsum("ij,jk,ik->i", query_augmented, ad_xtx_inverse, query_augmented)[0]
    )
    return {
        "leverage": leverage,
        "within_ad": bool(leverage <= ad_threshold),
    }
...
def plot_ad(ad):
    ...
    ax.scatter(
        np.arange(1, ad_n + 1), train_leverage, color="#2563eb", alpha=0.65,
        s=46, label="Training compounds"
    )
    ax.axhline(ad_threshold, color="#15803d", linestyle="--", linewidth=2.5,
    ...
    ax.set_ylabel("Leverage", fontsize=22, fontweight="bold", fontname="Times New Roman")
    ax.set_title("Applicability Domain (Leverage)", fontsize=24, fontweight="bold", fontname="Times New Roman")
    ...
    if st.button("Predict single compound"):
        try:
            ...
            st.markdown(
                ...
                f"(Leverage = {ad['leverage']:.3f}; threshold = {ad_threshold:.3f})</div>",
                unsafe_allow_html=True,
            )
...
with tab_batch:
    ...
    batch_size = st.number_input(
        "Batch size for descriptor calculation",
        min_value=1,
        max_value=100,
        value=25,
        step=5,
    ...
    if batch_file is not None:
        try:
            ...
            if "SMILES" in batch.columns:
                ...
                summary["AD_Leverage"] = np.nan
                summary["Within_AD"] = ""
                summary["AD_Threshold"] = ad_threshold
                summary["Descriptor_Source"] = ""
                summary["Descriptor_Error"] = ""

                total = len(summary)
                if total > 1000:
                    st.warning(
                        "Large SMILES files can take a long time on Streamlit Cloud. "
                        "For best stability, upload 500-1000 compounds at a time."
                    )
                progress = st.progress(0)
                status = st.empty()
                result_area = st.empty()
                smiles_values = batch["SMILES"].astype(str).tolist()

                for start in range(0, total, int(batch_size)):
                    end = min(start + int(batch_size), total)
                    status.info(f"Calculating compounds {start + 1}-{end} of {total}...")
                    for i in range(start, end):
                        try:
                            row, source = descriptors_from_user_smiles(smiles_values[i])
                            prediction = predict_one_feature_row(row)
                            for key, value in prediction.items():
                                summary.at[i, key] = value
                            summary.at[i, "Descriptor_Source"] = source
                        except Exception as exc:
                            summary.at[i, "Descriptor_Error"] = str(exc)
                    progress.progress(end / total)
                    result_area.dataframe(summary.iloc[max(0, end - 200):end], use_container_width=True)
                    gc.collect()

                status.success(f"Completed {total} compounds.")
            else:
                ...
                summary["AD_Leverage"] = predictions["AD_Leverage"].values
